...
//       ALL agents/modes/subagents, not just Grok.
...
export default function(amp: PluginAPI) {
	...
	// Tool list for the Grok agent.
	// web_search and read_web_page are transparently replaced by Firecrawl
	// if firecrawl-hook.ts is also installed (global hook).
	// The four firecrawl_* tools are available when firecrawl-hook.ts is loaded.
	...
	const agent = amp.experimental.createAgent({
		...
		display: { label: 'Grok', color: '#ea580c' },
	})
